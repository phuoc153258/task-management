...
function CreateTaskReport({ setShowModal, isFetchData, setIsFetchData, taskId }: any) {
    ...
    const handleCreateTaskReport = async () => {
        setShowLoading(true)
        try {
    ...
    return (
        <>
            ...
            <div className="flex items-center justify-end p-6 space-x-2 border-t border-gray-200 rounded-b dark:border-gray-600">
                ...
                <ModalButton title={<>{showLoading && <div className='absolute flex items-center left-1'><Loading /></div>}Create task report</>} isDisabled={showLoading} callback={() => { handleCreateTaskReport() }} styles={`px-8 py-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-blue-300 relative`} />
            </div>
        </>
    );
}
